...
class ClientSideEncryptionSpecTest extends FunctionalTestCase
{
    ...
    public function dataKeyProvider()
    {
        return [
            'local' => [
                function (ClientEncryption $clientEncryption, Client $client, Client $clientEncrypted, self $test) {
                    $commands = [];
                    $localDatakeyId = null;
                    ...
                    $this->assertInstanceOf(Binary::class, $localDatakeyId);
                    $this->assertSame(Binary::TYPE_UUID, $localDatakeyId->getType());

                    $this->assertCount(2, $commands);
                    $insert = $commands[1]['started'];
                    $this->assertSame('insert', $insert->getCommandName());
                    $this->assertSame('majority', $insert->getCommand()->writeConcern->w);
                    ...
                    $this->assertSame('local', $key['masterKey']['provider']);

                    $localEncrypted = $clientEncryption->encrypt('hello local', ['algorithm' => 'AEAD_AES_256_CBC_HMAC_SHA_512-Deterministic', 'keyId' => $localDatakeyId]);
                    $this->assertInstanceOf(Binary::class, $localEncrypted);
                    $this->assertSame(Binary::TYPE_ENCRYPTED, $localEncrypted->getType());
                    ...
                    $this->assertNotNull($helloLocal);
                    $this->assertSame('hello local', $helloLocal['value']);

                    $localEncryptedAltName = $clientEncryption->encrypt('hello local', ['algorithm' => 'AEAD_AES_256_CBC_HMAC_SHA_512-Deterministic', 'keyAltName' => 'local_altname']);
                    $this->assertEquals($localEncrypted, $localEncryptedAltName);
            ...
            'aws' => [
                function (ClientEncryption $clientEncryption, Client $client, Client $clientEncrypted, self $test) {
                    ...
                    $this->assertInstanceOf(Binary::class, $awsDatakeyId);
                    $this->assertSame(Binary::TYPE_UUID, $awsDatakeyId->getType());
                    ...
                    $key = $keys[0];
                    $this->assertNotNull($key);
                    $this->assertSame('aws', $key['masterKey']['provider']);

                    $awsEncrypted = $clientEncryption->encrypt('hello aws', ['algorithm' => 'AEAD_AES_256_CBC_HMAC_SHA_512-Deterministic', 'keyId' => $awsDatakeyId]);
                    $this->assertInstanceOf(Binary::class, $awsEncrypted);
                    $this->assertSame(Binary::TYPE_ENCRYPTED, $awsEncrypted->getType());
                    ...
                    $this->assertNotNull($helloAws);
                    $this->assertSame('hello aws', $helloAws['value']);

                    $awsEncryptedAltName = $clientEncryption->encrypt('hello aws', ['algorithm' => 'AEAD_AES_256_CBC_HMAC_SHA_512-Deterministic', 'keyAltName' => 'aws_altname']);
                    $this->assertEquals($awsEncrypted, $awsEncryptedAltName);
    ...
    public function testExternalKeyVault($withExternalKeyVault)
    {
        ...
        if (! $withExternalKeyVault) {
            ...
            $clientEncryption->encrypt('test', ['algorithm' => 'AEAD_AES_256_CBC_HMAC_SHA_512-Deterministic', 'keyId' => $keyId]);
        }
    }
    ...
    public function testBSONSizeLimitsAndBatchSplitting()
    {
        ...
        // Test 1
        $collection->insertOne(['_id' => 'over_2mib_under_16mibn', 'unencrypted' => str_repeat('a', 2097152)]);
    ...
    public function testCorpus($schemaMap = true)
    {
        ...
        $this->assertSameDocument($corpus, $corpusDecrypted);
    }
    ...
    public function testCustomEndpoint()
    {
        ...
        $encrypted = $clientEncryption->encrypt('test', ['algorithm' => 'AEAD_AES_256_CBC_HMAC_SHA_512-Deterministic', 'keyId' => $keyId]);
    ...
    private function insertKeyVaultData(array $keyVaultData = null)
    {
        ...
        $collection = $context->selectCollection('admin', 'datakeys', ['writeConcern' => new WriteConcern('majority')] + $context->defaultWriteOptions);
        $collection->drop();
